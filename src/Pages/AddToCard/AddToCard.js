...
function AddToCard() {
  ...
  const handleDelete = (id) => {
    ...
    if (proceed) {
      const url = `https://manufacturer-website-server-side-l833.onrender.com/addToCard/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
  ...
  const onSubmit = async (data) => {
    ...
    const url = `https://manufacturer-website-server-side-l833.onrender.com/orders`;
    fetch(url, {
      method: "POST",
      headers: {
      ...
      .then((result) => {
        if (result.acknowledged === true) {
          myOrders.map((mOs) => {
            const url = `https://manufacturer-website-server-side-l833.onrender.com/addToCard/${mOs._id}`;
            fetch(url, {
              method: "DELETE",
            })
              .then((res) => res.json())
